...
export default function Sidebar() {
  ...
  const handleNestedListToggle = (listName) => {
            ...
            {menuItems.map((item, index) => (
              <React.Fragment key={item.text}>
                <ListItem disablePadding sx={{ display: 'block' }}>
                  <ListItemButton
                    onClick={() => item.children ? handleNestedListToggle(item.text) : navigate(item.path)}
                    sx={{
                      minHeight: 48,
                      justifyContent: open ? 'initial' : 'center',
                      px: 2.5,
                      backgroundColor: isSelected(item.path) ? 'rgba(0, 0, 0, 0.08)' : 'transparent',
                    }}
                  >
                    <ListItemIcon
                      sx={{
                    ...
                    <ListItemText primary={item.text} sx={{ opacity: open ? 1 : 0, color: isSelected(item.path) ? 'primary.main' : 'inherit' }} />
                    {item.children && (openNestedList[item.text] ? <ExpandLess /> : <ExpandMore />)}
                  </ListItemButton>
                </ListItem>
                {item.children && (
                        ...
                        <ListItem key={child.text} disablePadding sx={{ pl: 4 }}>
                          <ListItemButton
                            ...
                            sx={{
                              ...
                              justifyContent: open ? 'initial' : 'center',
                              px: 2.5,
                              backgroundColor: isSelected(child.path) ? 'rgba(0, 0, 0, 0.08)' : 'transparent',
                            }}
                          >
                            <ListItemIcon
                              sx={{
                                ...
                                mr: open ? 3 : 'auto',
                                justifyContent: 'center',
                                color: isSelected(child.path) ? 'primary.main' : 'inherit',
                              }}
                            >
                              {child.icon}
                            </ListItemIcon>
                            <ListItemText primary={child.text} sx={{ opacity: open ? 1 : 0, color: isSelected(child.path) ? 'primary.main' : 'inherit' }} />
                          </ListItemButton>
                        </ListItem>
                      ))}
                    </List>
